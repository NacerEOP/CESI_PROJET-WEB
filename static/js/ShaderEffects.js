...
export class CurvedTrackEffect {
  async init(gl, app) {
    ...
    // Get baseUrl from <base> tag for correct shader path resolution
    const baseEl = document.querySelector('base');
    const prefix = baseEl ? baseEl.getAttribute('href') : '';
    const shaderUrl = (path) => prefix + path.replace(/^\/+/, '');

    this.program = new ShaderProgram(
      gl,
      shaderUrl("static/shaders/curved.vert"),
      shaderUrl("static/shaders/curved.frag")
    );
